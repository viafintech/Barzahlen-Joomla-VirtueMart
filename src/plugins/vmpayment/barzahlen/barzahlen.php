...
class plgVmpaymentBarzahlen extends vmPSPlugin {
  ...
  public function plgVmOnSelfCallFE() {
    ...
    if(isset($_GET['transaction_id'])) {
      $query = "SELECT virtuemart_paymentmethod_id, virtuemart_order_id
                ...
                AND amount = '".$_GET['amount']."' AND state = '" . self::STATE_PENDING . "'";
    }
  ...
  protected function _generateRefundTable($paymentTable) {
    ...
    if($refunded < $paymentTable->amount) {
      $html .= $this->getHtmlRowBE('BARZAHLEN_NEW_REFUND',
                                   $this->_renderRefundButton($paymentTable->virtuemart_order_id, $paymentTable->virtuemart_paymentmethod_id, $paymentTable->transaction_id, $paymentTable->currency)
